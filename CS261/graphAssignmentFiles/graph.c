...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	int i; /* need this to compile in flip */
	
	clearVisited(g); /* Clear the board of anything that it's done before */

	struct cirListDeque *stack = malloc(sizeof(struct cirListDeque)); /* Create a stack */
	initCirListDeque(stack); /* Initialize It */
	addFrontCirListDeque(stack, source); /* Add the current item to the front of the list */
	struct Vertex* currentVertex = malloc(sizeof(struct Vertex)); /* Create a placeholder Vertex */
	
	while(!isEmptyCirListDeque(stack)) {
		/* Pop the stack */
		currentVertex = frontCirListDeque(stack); /* Set the currentVertex to the one we added in above */
		removeFrontCirListDeque(stack);
		if(currentVertex == destination) { /* If we made it */
			return 1; /* End that sucka */
		}
		for(i = 0; i < currentVertex->numNeighbors; i++) { /* Go through the neighbors */
			if(currentVertex->neighbors[i]->isVisited == 0) { /* If the neighbor hasn't been visited */
				addFrontCirListDeque(stack, currentVertex->neighbors[i]); /* add it to the front of the queue */
				currentVertex->neighbors[i]->isVisited = 1; /* Mark it as visitied */
			}
		}
...
{
	/* FIXME you will write this */
	/* If I understand this correctly, it will be VERY close to the above function since we don't have a notion of left and right neighbors, we just have "total neighbors," so we still need to go through the neighbors 1 by 1 */
	...
	while(!isEmptyCirListDeque(stack)) {
		...
		for(i = 0; i < currentVertex->numNeighbors; i++) { /* Go through the neighbors */
			if(currentVertex->neighbors[i]->isVisited == 0) { /* If the neighbor hasn't been visited */
				addBackCirListDeque(stack, currentVertex->neighbors[i]); /* add it to the back of the queue */
				currentVertex->neighbors[i]->isVisited = 1; /* Mark it as visitied */
			}
		}
		currentVertex->isVisited = 1;

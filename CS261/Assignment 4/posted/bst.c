...
{
	if(cur == NULL) {
		struct Node *newNode = (struct Node *)malloc(sizeof(struct Node)); //Create and store space
		newNode->val = val;
		newNode->right = 0;
		newNode->left = 0;
		return newNode;
...
{
	if(cur->left == NULL) { //If there's nothing to the left
		return cur->val; //Return that ish
	}

	else { //If there is something to the left
		return _leftMost(cur->left); //Then call the same function moving onto the left one tho
	}
}
...
{
	if(cur->left != NULL) { //If there's still stuff to the left
...
{
	//Make sure them inputs are legit
	assert(cur != NULL);
	assert(val != NULL);

	if(compare(cur->val, val) == 0) {
		if(cur->right == NULL) {
			free(cur); //Free dat ish
			return NULL; //Don't return anything because that ish is freed
		}
		else {

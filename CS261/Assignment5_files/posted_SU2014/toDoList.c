...
{
	Task* newTask = malloc(sizeof(Task)); //Create a new one and hold the space for it
	strcpy(newTask->description, desc); //Add the description
	newTask->priority = priority; //Add the priority
	return newTask; //Return that sucka
}
...
{
	DynArr *temp; //Create a temporary array
	temp = createDynArr(sizeDynArr(heap)); //Create a dynamic array at the same size
	copyDynArr(heap, temp); //Copy the stuff over to the new array

	while(sizeDynArr(temp) > 0) { //Basically, print out the minimum item and then remove it from the heap and keep going until the size is 0.
		TYPE item = getMinHeap(temp);
		print_type(item);
		removeMinHeap(temp);
	}
}
...
{
	Task* test; //Using how the print_type did it. 
	Task* test2; //Using how the print_type did it
	test = (Task*)left; //Using how the print_type did it
	test2 = (Task*)right; //Using how the print_type did it
	if(test->priority == test2->priority) { //If the left is equal to the right
		return 0;
	}
	else if(test->priority > test2->priority) { //If the left is greater than the right
		return 1;
	}

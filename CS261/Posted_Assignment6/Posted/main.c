...
int main (int argc, const char * argv[]) {
	...
	fileptr = fopen(filename, "r"); //Open up the file

	char* word;
	int* val;

	while((word = getWord(fileptr))) {
		if(containsKey(hashTable, word)) { //If the word already exists
			val = atMap(hashTable, word);
			(*val) = (*val) + 1; //Increase the number
		}
		else { //If it doesn't already exist
			insertMap(hashTable, word, 1);  //Add it in
		}
	}
	fclose(fileptr); //And closed

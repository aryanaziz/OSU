...
int main(int argc, char *argv[]) {
  ...
  //Check if the file has any bad characters
  char c;
  FILE *fr = fopen(argv[1], "r");
  while((c = fgetc(fr)) != EOF) {
    if(c == '$') { //Set to just that for now, all it needs is an array for all bad chars, or an array of all good and check against that
  ...
  char type[2];
  bzero(type, 2);
  n = recv(sockfd,type,2,0); //Receive the server's type
  if(type[0] == 'e') { //If it's the encode server
    sendText(sockfd, argv[1]); //Send the text file
    char confirmation[2];
    bzero(confirmation, 2);
    n = recv(sockfd,confirmation,2,0); //Wait to get confirmation that server got it
    if(confirmation[0] == '1') { //If the server sent confirmation
      sendKey(sockfd, argv[2]); //Send the key file
    }
    ...
    else { //If it was not the encode server

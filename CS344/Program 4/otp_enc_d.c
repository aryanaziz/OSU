...
void getKey(int socket, int processId) {
	...
	fclose(fr);
	//printf("File '%s' Transfer Complete\n", fr_name);
	//Don't worry about num chars here, we will use the text file for everything
	return;
}
...
void encode(int processId, int numChars, int socket) {
	...
	while((c = fgetc(fr)) != EOF && i < numChars - 1) { //Read it in character by character
		fileArray[i] = c; //Write it to an array
		i++;
	}
	fclose(fr);
	...
	while((d = fgetc(kr)) != EOF && j < numChars - 1) { //Read it in character by character
		keyArray[j] = d; //Write it to an array
		j++;
	}
	fclose(kr);
	...
	for(i = 0; i < numChars - 1; i++) { //Iterate through the textfile array
		int sum = 0;
		for(j = 0; j < 28; j++) { //And iterate through each of the possible letters
			if(fileArray[i] == letters[j]) { //If the current letter matches one of the possible letters
				sum = sum + j + 1; //Grab the position in the array and add it to the sum
			}
			if(keyArray[i] == letters[j]) { //If the key array letter matches one of the possible letters
				sum = sum + j + 1; //Grab the position in the array and add it to the sum
			}
		}
		int remainder = sum % 27; //Do the modulo operation
		encodedArray[i] = letters[remainder]; //Set the encoded letter to be the value at the remainder index
		//printf("%c", letters[remainder]);
	}

	//Get the encoded message together
	...
	unlink(fr_name); //Delete the temporary file
	unlink(kr_name); //Delete the temporary file
	return;
}
...
void newThread(int socket, int processId) {
	...
	bzero(type, 2);
	sprintf(type, "e");
	n = write(socket,type,strlen(type)); //Authenticate that it's encode server
	//printf("Getting file from pid %d\n", processId);
	int numChars = getFile(socket, processId); //Get the file and store the number of characters
	char confirmation[2];
  	bzero(confirmation,1);
  	sprintf(confirmation, "1"); //Send the confirmation
...
int main(int argc, char *argv[]) {
	...
	while(1) {
		newsockfd = accept(sockfd, (struct sockaddr *) &cli_addr, &clilen); //Accept any sockets
		processId = fork(); //Creating a new thread
		if(processId == 0) { //It was able to create a new fork
			close(sockfd);
			int childProcess = getpid(); //grab child process id, used for temp file creation
			//printf("Process Id: %d\n", childProcess);
